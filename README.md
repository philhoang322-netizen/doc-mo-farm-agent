...
**Lịch sử hội thoại và FB-DV.** Inbound messages, Messenger echoes (`message_echoes`), and replies this app sends are stored in `conversation_messages` (migration `033_conversation_messages.sql`). Bodies stay in the database and are masked again before any Claude call. A Page thread is DV when a page reply is attributed to Lành (`from.name` when Graph sends it, otherwise a whole-word signature such as "Lành"; "trời lạnh" does not match). `POST /admin/api/fb/backfill?months=3` pages Graph `/{page-id}/conversations?platform=messenger` in the background (default 3 months, maximum 6; manager session or `X-Admin-Key`). `GET /admin/api/fb/backfill/status` returns counts and sender-field names only, never message text. `POST /admin/api/fb/relabel` recomputes `thread_labels` locally (no Claude call) and moves pending drafts between FB-Sale and FB-DV. `GET /admin/api/fb/labels/stats` returns label counts and up to 30 DV keywords. `GET /admin/api/fb/labels/sample?label=dv&reason=signature&limit=30` is read-only: for each sampled thread it returns the label, the matched Lành substring with about 40 characters of the page message around it, a 60-character first customer line, whether a catalog product name appears, plus a histogram of those substrings and a product-vs-service count for every DV-by-signature thread. It does not change labels. A manual **Chuyển qua Sale / DV** writes a manual thread label. A live uncertain thread may call Claude with at most 8 sanitized examples. Nothing here auto-sends.
