...
Each inbox card header shows the channel display name when it can be fetched: **Tên Zalo** from the OA user detail API, or **Tên FB** from Graph `GET /{PSID}?fields=first_name,last_name,name,profile_pic` with `FB_PAGE_ACCESS_TOKEN`. The avatar is shown when the URL is https. A permission error or a missing token leaves the old name in place and does not block the card. Names are cached on the customer (`customers.channel_names`, migration 027) and refreshed at most once a day; opening the inbox backfills older conversations. If a later link stores both sources on the same customer, the header shows both. The KiotViet form starts with that name and the hint “lấy từ Tên Zalo” or “lấy từ Tên FB”. A name the manager types, or a KiotViet customer already matched by phone, wins. A newly created KiotViet customer gets `Zalo: <name>` or `FB: <name>` in the customer comments.
