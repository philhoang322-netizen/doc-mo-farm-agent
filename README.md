...
| `ALERT_ZALO_USER_ID` | unset | Zalo user id that receives connection alerts through the OA. The OA can refuse a user outside the allowed interaction window, so set Telegram as well. |
| `TELEGRAM_BOT_TOKEN` | unset | Optional. Used only together with `TELEGRAM_CHAT_ID`. The token is never logged or returned by `/health`. |
| `TELEGRAM_CHAT_ID` | unset | Optional Telegram chat for the same alerts. |
| `ALERT_THROTTLE_MIN` | `45` | Same alert at most this often (clamped 30–60). A recovered alert sends once when the condition clears. |
| `ALERT_VERIFY_THRESHOLD` | `5` | Messenger signature or Graph-verify failures in `ALERT_VERIFY_WINDOW_MIN` before an alert. |
| `ALERT_VERIFY_WINDOW_MIN` | `15` | Window for that burst. |
| `ALERT_SILENCE_HOURS` | `6` | After a channel has received traffic, no new Messenger or Zalo webhook for this long during 07:00–22:00 ICT raises an alert. |
...
**Lịch sử hội thoại và FB-DV.** Inbound messages, Messenger echoes (`message_echoes`), and replies this app sends are stored in `conversation_messages` (migration `033_conversation_messages.sql`). Bodies stay in the database and are masked again before any Claude call. The topic of the customer's latest message that mentions a topic decides the label. Page text is used only when the customer has no topic. Service wording is DV. Shipping, an order, or a catalog product name is Sale, even when Lành signed the reply. Price words (giá, mua, đặt, cọc, tiền) do not set a topic, so a later price question keeps the earlier topic. A message that mentions both a stay and a product stays DV unless it also has an order or shipping word. FAQ topics that are not products are not loaded as product names. Bare vé and bare đêm are not service hits. When no topic is found, a Lành sign-off — the name as the last word, or a line that is only the name, allowing ạ/nha/nhé/ạa; an inline mention and "trời lạnh" do not match — labels the thread DV. A thread with no topic and no sign-off stays unknown. A brand-new live turn with no topic still defaults to Sale. `from.name` does not select the label. DV keywords are learned only from threads labeled DV by a service topic, never from a sign-off, and catalog product names plus giá, xin giá, ship, đặt, and mua are left out. A manual Sale or DV label wins. `POST /admin/api/fb/backfill?months=3` pages Graph `/{page-id}/conversations?platform=messenger` in the background (default 3 months, maximum 6; manager session or `X-Admin-Key`). `GET /admin/api/fb/backfill/status` returns counts and sender-field names only, never message text. `POST /admin/api/fb/relabel` recomputes `thread_labels` locally (no Claude call) and moves pending drafts between FB-Sale and FB-DV. `GET /admin/api/fb/labels/stats` returns label counts, up to 30 DV keywords, the manual-move count, and the top learned n-grams for Sale and DV. It does not return the customer text those n-grams came from. `GET /admin/api/fb/labels/sample?label=dv&reason=signature&limit=30` is read-only: for each sampled thread it returns the label, the matched Lành substring with about 40 characters of the page message around it, a 60-character first customer line, whether a catalog product name appears, plus a histogram of those substrings and a product-vs-service count for every DV-by-signature thread. It does not change labels. The same response includes `rule_counts`, the label and reason totals the current rule would write, plus `manual_moves` and `learned_ngrams`. A manual **Chuyển qua Sale / DV** (one tap, no confirm, on every card, including Zalo) moves every draft in that customer thread, keeps later messages on the chosen line, and stores a training row in the database. Relabel does not overwrite that manual label. N-grams from those rows add capped, decaying weight on top of the topic rule, and a catalog product name only adds Sale weight. The weights are recomputed on relabel. A live uncertain thread may call Claude with at most 8 sanitized examples. Facebook system notices (a reply to a post, ad, story, comment, or reel, and the automatic-greeting notice) are not Shop lines: a post reply is a short link, the greeting notice is hidden, and neither is used for labels, keyword learning, n-gram weights, or the draft prompt. Nothing here auto-sends.

Inbox groups (Zalo OA, FB-Sale, FB-DV) are columns `biz_line`, `biz_sticky`, `deleted_at`, and `source_msg_id` on `outbound_drafts` (migration `022_inbox_groups.sql`, also applied at startup). **Xóa tin này** and **Xóa cả cuộc chat** sit on the card and the detail action row. There is no confirm. The card hides immediately and a **Hoàn tác** toast lasts 3 seconds; after that the row is hard-deleted, an audit row is written, and a tombstone (`028_deleted_messages.sql`) stops that same source message from returning on the 20-second refresh, a webhook retry, or Đồng bộ tin bị sót. A later customer message with a new id creates a new card. Delete does not send anything and does not call KiotViet. Only a manager can delete. **Duyệt & Gửi** sends on click, with no confirm. Card times use `source_received_at` when Messenger or Zalo sent a clock (`029_source_received_at.sql`), and `created_at` with a tilde when they did not. Each card shows earlier Facebook turns as bubbles above the latest customer line. A manual **Chuyển qua Sale / DV** is sticky for later messages from that customer. The inbox polls about every 20 seconds while the tab is visible. That poll only fetches data. It never reloads the page. While a Tạo đơn KiotViet form is open, a reply or quick-entry field is being edited, the detail pane is open, or any input is focused, the card list is not inserted into, reordered, or rebuilt. Tab counts still update, and **Có X tin mới — bấm để hiện** applies the new cards only when clicked. When nothing is being edited, new cards are inserted and the card at the top of the viewport stays where it is.

`GET /health` returns per-integration status (`messenger`, `zalo`, `kiotviet`, `graph`, `llm`, `pipeline`) with no secrets and no customer text. It is HTTP 503 while an alert is active so an uptime monitor can watch it. The same picture is on `/admin` (banner, last success in ICT) and `GET /admin/api/health` (password session). Alerts also go to `ALERT_ZALO_USER_ID` via the OA sender and, when both Telegram variables are set, to Telegram. Phrases are fixed. They do not include tokens or message text.

Status folders sit beside those groups: **Chờ xử lý**, **Đã gửi** (successful Approve & Send), **Đã mua** (KiotViet create, or manual **Đã chốt** on FB-DV), **Do dự** (manual, or `INBOX_HESITANT_HOURS` after Đã gửi with no order and no newer customer message), and **Từ chối** (manual only). A new customer message opens a Chờ xử lý draft tagged `trước: …` and does not move an older Đã mua draft. Older soft-deleted cards are only in **Đã xóa**. A new **Xóa tin này** or **Xóa cả cuộc chat** does not land there. Each card shows an editable reply (`ai_suggested_draft`), **Duyệt & Gửi**, and **Cho AI học từ câu trả lời này** (on by default). Checked sends store a `training_logs` row: an edit is a correction (weight 1); an unchanged approval is an approved example (weight 0.35) and ranks lower in the model prompt. Unchecked stores no example and the audit row says `learning: off`. Migrations `023_training_example_weight.sql` and `024_inbox_folders.sql`.
...
A phone number is the key that ties one person together. When a number shows up in a message, or a manager creates a KiotViet invoice from a card, the Zalo user id or Facebook PSID on that card is stored on the same profile. The manager can also type a number on the card and press **Gắn**, or **bỏ** a channel. Nothing here sends a reply. Drafts stay `PENDING_REVIEW`.

Each inbox card shows the name, phone, linked channels (Zalo, Facebook, KiotViet), and a short purchase summary: last invoices, total spent, last purchase date, and unpaid invoices. The summary is cached (`CUSTOMER_HISTORY_CACHE_MIN`, default 15 minutes) and read from KiotViet with the existing client id, secret, and retailer. The same summary, with the phone and name removed, is added to the AI draft context so the reply can mention a returning customer. The copy sent to the model goes through the existing PII mask.

Profiles live in Postgres table `customer_links` when `DATABASE_URL` is set (migration 026). Without a database they sit in a JSON file beside the drafts and do not survive a restart. `customers` / `customer_identities` still merge memory by phone when the database is on.
