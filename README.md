...
**Xóa** does not ask for confirmation. The card leaves the list immediately and a toast with **Hoàn tác** stays for 3 seconds. Each click has its own toast and timer. **Hoàn tác** puts that card back in the same place with the same status and group, and does not call the server. If the 3 seconds pass, the browser sends `POST /admin/api/drafts/:id/delete`. That hard-deletes the draft row and dependent `training_logs` and `staff_handoffs` rows for that draft. The card is gone from every folder. There is no **Đã xóa** folder, and nothing can be restored after the 3 seconds.

Closing the tab or refreshing during those 3 seconds drops the in-browser timer, so the DELETE is never sent. The draft is still complete on the server and a full reload shows the card again. A background refresh while the timer is running does not insert that card back into the list.

The server keeps a tombstone of channel, source message id, `deleted_by`, and `deleted_at` (migration `028_deleted_messages.sql`, also created at startup). It does not store the message body. **Đồng bộ tin bị sót** and a webhook retry both see the tombstone and do not create the draft again. The audit row records who, when, channel, and customer id, and not the message body. Delete does not call Facebook or Zalo and does not send anything.

**Duyệt & Gửi** sends the text currently in the reply box as soon as it is clicked. There is no confirm dialog. The result toast is **Đã gửi · AI đã học** or **Đã gửi · không học**, and the card moves to **Đã gửi**. An empty reply shows an inline error and does not send. The button stays disabled while that request is in flight, and the server sends a draft at most once. **Xác nhận tạo hoá đơn** / **Xác nhận tạo đơn đặt hàng** still asks before anything is posted to KiotViet. Sale and DV still cannot send unless a manager turns staff sending on. The **Đã xóa** folder mentioned with the status folders is gone: after the 3 seconds there is nothing to restore.

## Thời gian trên thẻ

The clock next to the customer name is when Messenger or Zalo received the customer's message (`source_received_at`, migration `029_source_received_at.sql`), shown as **Nhận: HH:mm dd/MM/yyyy** in Asia/Ho_Chi_Minh. Messenger uses `messaging[].timestamp`. A backfilled Graph message uses `created_time`. Zalo OA uses the webhook event `timestamp`. If the source sent no time, the card uses `created_at` and prefixes the clock with **~**. A relative phrase such as "1 phút trước" stays on the tooltip.

After **Duyệt & Gửi** succeeds, `sent_at` is the time the send call returned success, and `send_message_id` is the id Graph or Zalo returned when it sent one. The card shows **Gửi: HH:mm dd/MM/yyyy ·** the sender under the reply, including in **Đã gửi**, **Đã mua**, **Do dự**, and **Từ chối** when that send time exists. The name is the admin user (`sale:lan` shows `lan`) when one is stored, otherwise **Quản lý**. The same send time and sender are on the `draft.sent` audit row. Nothing is sent automatically.
