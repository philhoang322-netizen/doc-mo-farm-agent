...
**Lịch sử hội thoại và FB-DV.** Inbound messages, Messenger echoes (`message_echoes`), and replies this app sends are stored in `conversation_messages` (migration `031_conversation_messages.sql`). Bodies stay in the database and are masked again before any Claude call. A Page thread is DV when a page reply is attributed to Lành (`from.name` when Graph sends it, otherwise a signature such as "Lành"). `POST /admin/api/fb/backfill?months=6` pages Graph `/{page-id}/conversations?platform=messenger` in the background (manager session or `X-Admin-Key`). `GET /admin/api/fb/backfill/status` returns counts only. `POST /admin/api/fb/relabel` recomputes `thread_labels` and moves pending drafts between FB-Sale and FB-DV. `GET /admin/api/fb/labels/stats` returns label counts and up to 30 DV keywords. A manual **Chuyển qua Sale / DV** writes a manual thread label. Nothing here auto-sends.
